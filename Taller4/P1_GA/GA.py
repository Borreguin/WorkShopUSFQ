...
class GA:
    ...
    def run(self):
        ...
        for _ in range(self.n_iterations):
            ...
            aptitudes = [evaluate_aptitude(
                self.evaluation_type, individual, self.objective) for individual in self.population]
            ...
            if best_individual == self.objective:
                ...
                print(f"Generación {self.n_generation}: {
                      best_individual} - Aptitud: {best_aptitude}")
                break
            print(f"Generación {self.n_generation}: {
                  best_individual} - población: {len(self.population)} - Aptitud: {best_aptitude}")
        ...
        if not success:
            print(f"Objetivo no alcanzado en las iteraciones establecidas {
                  self.n_iterations}")
...
def case_study_2(_objetive):
    ...
    ga.set_best_individual_selection_type(
        BestIndividualSelectionType.MIN_DISTANCE)
...
def case_study_4(_objetive):
    ...
    population = generate_population(n_population, len(_objetive))
    mutation_rate = 0.01
    n_iterations = 1000
    ga = GA(population, _objetive, mutation_rate, n_iterations)
    ga.set_evaluation_type(AptitudeType.BY_DISTANCE)
